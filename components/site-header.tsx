...
export function SiteHeader() {
  ...
  return (
    <>
      {/* 64px spacer so fixed header doesn't overlap content */}
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 w-full border-b border-border/40 bg-background/80 backdrop-blur-xl transition-transform duration-500 ease-[cubic-bezier(0.25,0.46,0.45,0.94)] ${visible ? 'translate-y-0' : '-translate-y-full'}`}>
        ...
        <div className="flex h-16 w-full items-center justify-between px-5 md:px-8">
          ...
          <Link href="/" className="group flex items-center gap-3" aria-label="Mathe Rodrick Attorneys Inc home">
            <span className="flex h-11 w-11 items-center justify-center border border-border/50 bg-background/50 text-lg font-thin tracking-widest text-foreground shadow-sm transition-all group-hover:border-foreground/40 group-hover:shadow-[0_0_15px_rgba(255,255,255,0.05)]">
              RM
            </span>
            <span className="hidden flex-col leading-none sm:flex">
              <span className="text-sm font-light tracking-[0.18em] text-foreground">MATHE RODRICK</span>
              <span className="text-[0.6rem] font-light tracking-[0.4em] text-muted-foreground">ATTORNEYS INC</span>
            </span>
          </Link>
          ...
          <nav className="hidden items-center gap-8 lg:flex" aria-label="Primary">
            <Link
              href="/"
              className="relative text-[11px] font-light uppercase tracking-[0.25em] text-muted-foreground transition-all hover:text-foreground"
            >
              Home
            </Link>
            ...
            >
              <Link
                href="/our-services"
                className={`flex items-center text-[11px] font-light uppercase tracking-[0.25em] transition-all hover:text-foreground ${desktopServicesOpen ? 'text-foreground' : 'text-muted-foreground'}`}
              >
                Our Services
              </Link>
                ...
                <div className="flex flex-col py-3">
                  {services.map((s) => (
                    <Link
                      key={s.href}
                      href={s.href}
                      className="group flex items-center px-6 py-3 text-[11px] font-light uppercase tracking-[0.15em] text-muted-foreground transition-all hover:bg-foreground/5 hover:text-foreground"
                    >
      ...
      >
        ...
        <div className="flex h-16 shrink-0 items-center justify-between border-b border-border/30 px-6">
          <span className="text-[10px] font-light uppercase tracking-[0.3em] text-muted-foreground">Menu</span>
          <button
            type="button"
            onClick={() => setMobileOpen(false)}
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden">
          <div className="flex flex-col py-4">
            <Link
              href="/"
              onClick={() => setMobileOpen(false)}
              className="group flex items-center border-b border-border/20 px-6 py-5 text-sm font-light uppercase tracking-[0.2em] text-foreground/90 transition-colors hover:bg-foreground/5 hover:text-foreground"
            >
            ...
            <div className="border-b border-border/20">
              <button
                ...
                className={`group flex w-full items-center px-6 py-5 text-sm font-light uppercase tracking-[0.2em] transition-colors hover:bg-foreground/5 ${mobileServicesOpen ? 'text-foreground' : 'text-foreground/90 hover:text-foreground'}`}
              >
                  ...
                  <div className="flex flex-col py-2">
                    {services.map((s) => (
                      <Link
                        ...
                        href={s.href}
                        onClick={() => setMobileOpen(false)}
                        className="group flex items-center px-10 py-3.5 text-xs font-light uppercase tracking-[0.15em] text-muted-foreground transition-colors hover:text-foreground"
                      >
        ...
        <div className="shrink-0 border-t border-border/30 bg-background p-6">
          <a
            href="tel:+27739083025"
            className="group flex items-center justify-center gap-3 border border-border/50 bg-foreground/5 py-4 text-xs font-light tracking-[0.2em] text-foreground transition-all hover:bg-foreground hover:text-background hover:shadow-lg"
          >
